...
function deleteItem(url, description) {
    if (description == null || description == "") {
        description = "آیا از Delete اطمینان دارید ؟";
    }
    Swal.fire({
        title: "هشدار !!",
        text: description,
        ...
        preConfirm: () => {
            ...
            $.ajax({
                ...
                error: function (data) {
                    ErrorAlert("مشکلی در اoperation رخ داده", "لطفا در Time دیگری امتحان کنید");
                }
            }).done(function (data) {
                ...
                if (data.Status === 200) {
                    Swal.fire({
                        title: data.Title,
                        text: data.Message == null ? "اoperation با موفقیت انجام شد" : data.Message,
                        icon: "success",
...
function Question(url, QuestionTitle, QuestionText, successText, callBack) {
    if (QuestionTitle == null || QuestionTitle == "undefined") {
        QuestionTitle = "آیا از انجام operation اطمینان دارید؟";
    }
...
function Success(Title, description, isReload = false) {
    if (Title == null || Title == "undefined") {
        Title = "operation با موفقیت انجام شد";
    }
...
function ErrorAlert(Title, description, isReload = false) {
    if (Title == null || Title == "undefined") {
        Title = "مشکلی در operation رخ داده است";
    }
    if (description == null || description == "undefined") {
        description = "";
    }
